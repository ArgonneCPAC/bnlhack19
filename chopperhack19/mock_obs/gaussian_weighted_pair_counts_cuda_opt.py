...
__all__ = (  # noqa
    ...
    'count_weighted_pairs_3d_cuda_noncuml',
    'count_weighted_pairs_3d_cuda_2d_noncuml')
...
@cuda.jit(fastmath=True)
def count_weighted_pairs_3d_cuda_2d_noncuml(
        x1, y1, z1, w1, x2, y2, z2, w2, _rbins_squared, result):
    starti, startj = cuda.grid(2)
    stridei, stridej = cuda.gridsize(2)
    ...
    for i in range(starti, n1, stridei):
        for j in range(startj, n2, stridej):
            dx = x1[i] - x2[j]
            dy = y1[i] - y2[j]
            dz = z1[i] - z2[j]
            dsq = cuda.fma(dx, dx, cuda.fma(dy, dy, dz * dz))
